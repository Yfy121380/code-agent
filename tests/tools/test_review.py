...
These tests cover the serial review child, read-only boundary, inherited
temporary read permissions, exclusive tool-call rule, and review progress UI.
"""
...
from codemate.runtime.review import MANUAL_REVIEW_REQUEST, REVIEW_ALLOWED_TOOLS, manual_review_request
from codemate.tools import DELEGATE_TOOL_SPEC, SUBAGENT_MAX_STEPS
...
def test_review_child_runtime_configuration_is_fixed(tmp_path, monkeypatch):
    captured = {}
    ...
    result = agent.run_tool(
        ...
        {
            "task": (
                "Review the implementation for correctness while preserving "
                "existing behavior."
            )
        },
    )
    ...
    assert captured["approval_policy"] == "read_only"
...
def test_review_child_is_read_only_and_returns_report(tmp_path):
    ...
    agent = build_agent(
        ...
        [
            ModelResponse.tool_call(
                ...
                {
                    "task": (
                        "Verify the change preserves existing behavior and does "
                        "not write files during review."
                    )
                },
            ),
...
def test_review_child_inherits_temporary_read_permissions(tmp_path):
    ...
    result = agent.run_tool(
        ...
        {
            "task": (
                "Review the current changes and use the approved external note "
    ...
    mixed = ModelResponse.from_tool_calls(
        [
            ModelToolCall.create(
                ...
                {
                    "task": (
                        "Review the current changes for correctness while "
...
def test_review_runtime_has_dedicated_prompt_and_tools(tmp_path):
    ...
    result = agent.run_tool(
        ...
        {
            "task": (
                "Ensure the implementation handles the intended behavior and "
                "keeps adjacent behavior stable."
            )
        },
    )
    ...
    assert "You are an independent code reviewer" in prompt
    assert "Do not draw conclusions from an isolated diff hunk." in prompt
...
def test_review_is_visible_only_in_normal_main_agent(tmp_path):
    ...
    assert "Give the review tool a concrete objective" in agent.prefix
...
def test_manual_review_request_only_adds_nonempty_focus():
    ...
    assert prompt.endswith("User-requested review focus:\n重点检查权限绕过")
